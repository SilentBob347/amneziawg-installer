...
- [🆕 AmneziaWG 3.0 для self-hosted](#awg3-adv)
  - [Какую линию модуля вы получите](#awg3-lines-adv)
  - [Что 3.0 меняет на проводе, а что нет](#awg3-wire-adv)
  - [Новые параметры 3.0](#awg3-params-adv)
  - [Что обязано совпадать, а что нет](#awg3-must-match-adv)
  - [Три вещи, на которых легко потерять вечер](#awg3-gotchas-adv)
  - [Чего установщик пока не включает и почему](#awg3-not-yet-adv)
- [⚙️ Детали конфигурации клиента](#config-details-adv)
  - [AllowedIPs](#allowedips-adv)
  - [Что видно сайту при делении трафика по адресу назначения](#split-detect-adv)
...
- [🔒 Настройки безопасности сервера](#security-adv)
  ...
  - [Смена серверного ключа](#server-key-rotation-adv)
...
- [🤖 JSON-интерфейс для автоматизации (--json)](#json-api-adv)
...
<a id="awg3-adv"></a>
## 🆕 AmneziaWG 3.0 для self-hosted

В конце июля 2026 команда Amnezia выпустила **AmneziaWG 3.0** и переключила на неё PPA. Если у вас x86 и ядро 6.7 или новее, установщик **уже ставит модуль третьей версии** - специально ничего делать не нужно, и старые конфиги при этом продолжают работать.

Проверить, что стоит именно у вас:

```bash
awg --version                          # amneziawg-tools v3.0.20260730
cat /sys/module/amneziawg/version      # 3.0.20260731-04 - версия ЗАГРУЖЕННОГО модуля
```

⚠️ Спрашивайте именно загруженный модуль, а не файл на диске. `modinfo amneziawg | grep ^version` читает метаданные того `.ko`, который выбран на диске, и если на хосте оказались два дерева с модулем одного имени (такое возможно на ARM: закреплённый 2.0 в `extra/` и DKMS-3.0 в `updates/dkms/`), он назовёт не тот, что работает в ядре. `modinfo` пригодится в одном случае - когда модуль не загружен и `/sys/module/amneziawg/` отсутствует; тогда он покажет, что лежит на диске. `manage check` и `manage diagnose` с v5.25.0 спрашивают загруженный модуль и переходят к `modinfo` только как ко второму пути.

⚠️ Версии пакетов apt на этот вопрос не отвечают. Там `amneziawg-dkms` значится как `1.0.0-0~202608010147+c78a89e~ubuntu24.04.1`, а `amneziawg-tools` как `1.0.20210914-0~...`: начало строки - это версия упаковки, а настоящее содержимое - в суффиксе с хешем коммита. Две команды выше отвечают прямо.

<a id="awg3-lines-adv"></a>
### Какую линию модуля вы получите

| Условие | Что ставится | Протокол |
|---|---|---|
| x86_64, ядро >= 6.7 | `amneziawg-dkms` из `ppa:amnezia/ppa` | **3.0** |
| любая арх., ядро старее 6.7 (Debian 12 с 6.1) | закреплённый модуль из исходников | 2.0 |
| ARM64 / armhf, есть готовый пакет под ваше ядро | наш предсобранный пакет | 2.0 |
| ARM64 / armhf, готового пакета нет, ядро >= 6.7 | `amneziawg-dkms` из PPA | **3.0** |

Последняя строка не теория: готовые ARM-пакеты собираются для Raspberry Pi 3/4/5, Ubuntu 24.04 и 25.10 ARM64 и Debian 12/13 ARM64. Ubuntu 26.04 ARM64 в этот список пока не входит, и на таком хосте установщик, не найдя совпадения, уходит на обычный путь DKMS и ставит модуль из PPA, то есть третью версию.

На старых ядрах мы выбираем 2.0 **сознательно**, а не потому, что 3.0 там не собирается. Первые сутки после выхода она действительно не собиралась на ядре 6.1, upstream починил это 31 июля, и сейчас сборка проходит. Но именно на старых ядрах линия 3.0 обкатана меньше всего, а закреплённый 2.0 сверяется по неизменяемому коммиту. Порог снимем после отдельной проверки, а не по факту «сборка снова проходит».

На ARM установщик сначала пробует предсобранный пакет, и если находит подходящий под ваше ядро, до PPA дело не доходит вовсе. Предсобранные пакеты закреплены на 2.0. Если совпадения нет, дальше решает то же правило, что и на x86: ядро старее 6.7 - закреплённый 2.0 из исходников, ядро новее - модуль из PPA.

> ⚠️ **Закреплённая 2.0-линия не обновляется, и это осознанное решение, а не недоделка.** Модуль на закреплённом пути собирается из неизменяемого тега `v1.0.20260725`, а линия `1.0.x` у upstream заморожена - новых выпусков в ней не будет. То же относится к предсобранным ARM-пакетам.
>
> Что это значит на практике. На Debian 12 со штатным ядром 6.1 и на ARM с подходящим предсобранным пакетом вы получаете проверенный, но зафиксированный модуль. Пути «обновиться до третьей версии обычным `apt upgrade`» для таких хостов нет: установщик держит `amneziawg-dkms` в `apt-mark hold` именно для того, чтобы модуль из PPA не встал рядом вторым деревом с тем же именем. Ничего не ломается и работать не перестаёт - обновления просто не приходят.
>
> Если третья версия нужна именно на таком хосте, поддерживаемый путь один: получить ядро 6.7 или новее (на Debian 12 это ядро из backports) и переустановить. На x86 после этого начнётся обычный путь PPA. ⚠️ На ARM учтите порядок: предсобранный пакет пробуется первым, поэтому если под новое ядро найдётся готовый пакет, вы снова получите закреплённую 2.0 - третья версия приедет только тогда, когда совпадения не найдётся.

<a id="awg3-wire-adv"></a>
### Что 3.0 меняет на проводе, а что нет

Ничего из того, что вы уже настроили. Проверено замером: на модуле 3.0 одновременно поднимаются и работают четыре интерфейса с конфигами разных поколений - без единого AWG-параметра, в стиле 1.x, в стиле 2.0 и с параметрами 3.0.

Ключевое устройство: `H1`-`H4` внутри модуля стали диапазонами, но интерфейс, где они не заданы, получает диапазоны из одного значения - ровно типы сообщений WireGuard. Это видно и в проводе:

| Интерфейс | Что уходит в провод |
|---|---|
| без AWG-параметров | на смещении 0 байты `01 00 00 00`, то есть обычный WireGuard |
| `S1 = 100`, `H1 = 1234567` | на смещении 100 значение `H1`, перед ним junk-пакеты по `Jc` |

Скалярное значение `H1 = 12345` из конфига 2.0 разбирается как диапазон из одного числа, и в провод уходит ровно оно. Поэтому апгрейд модуля сам по себе ничего вашим клиентам не ломает.

<a id="awg3-params-adv"></a>
### Новые параметры 3.0

По сравнению с прошлой версией `amneziawg-tools` добавилось семь ключей конфигурации и **не удалено ни одного**: всё из 2.0 (`Jc`/`Jmin`/`Jmax`, `S1`-`S4`, `H1`-`H4`, `I1`-`I5`) принимается по-прежнему.

| Параметр | Что делает | Обязан совпадать у сервера и клиента? |
|---|---|---|
| `HeaderProtectionKey` | шифрование заголовков ChaCha20 | **да, иначе рукопожатия не будет вовсе** |
| `ContentPaddingAddition` | добавочный паддинг внутри шифрованной части | нет |
| `RekeyAfterTime` | когда начинать перезаключение ключей | нет |
| `RekeyTimeout` | пауза между попытками рукопожатия | нет |
| `RejectAfterTime` | когда считать сессию просроченной | нет |
| `KeepaliveTimeout` | пауза перед служебным пакетом | нет |
| `MaxHandshakeAttempts` | сколько попыток рукопожатия делать | нет |

Все семь принимаются и через `awg-quick`: он передаёт в `awg setconf` любые ключи, которые не разбирает сам, так что рукописные конфиги ничего особенного не требуют.

<a id="awg3-must-match-adv"></a>
### Что обязано совпадать, а что нет

Это место чаще всего понимают неправильно, потому что README самого модуля ядра описывает только 1.x/2.0, а README `amneziawg-go` делит параметры на «серверные» и «клиентские» без подробностей.

- **`S1`-`S4` обязаны совпадать.** Это не просто паддинг: приёмник читает тип сообщения **по смещению**, равному своему `S`. Разошлись значения - пакет не опознан.
- **`H1`-`H4` должны быть совместимы.** Отправитель берёт значение из своего диапазона, приёмник проверяет, что оно попадает в его. Проще держать их одинаковыми.
- **`HeaderProtectionKey` обязан быть одинаковым.** Проверено: ключ только на сервере, только на клиенте или разные ключи - рукопожатия нет вообще, 100% потерь. Это не постепенное ухудшение, а переключатель: как только сервер получил ключ, все пиры без него отваливаются.
- **`Jc`/`Jmin`/`Jmax` и `I1`-`I5` совпадать не обязаны.** Пакеты-пустышки и маскировочные пакеты шлёт инициатор рукопожатия, вторая сторона их просто игнорирует.

<a id="awg3-gotchas-adv"></a>
### Три вещи, на которых легко потерять вечер

**`HeaderProtectionKey` требует `S1`-`S4` не меньше 12.** Nonce для шифрования заголовка нигде не передаётся, а берётся из первых 12 байт S-паддинга, поэтому при меньшем значении конфиг отвергается. Ровно 12 подходит, хотя сообщение в ядре говорит «must be more then 12».

**Само сообщение по умолчанию не видно.** `awg` печатает только `Unable to modify interface: Invalid argument`, а объяснение уходит в отладочный лог ядра. Чтобы его увидеть:

```bash
echo "module amneziawg +p" > /sys/kernel/debug/dynamic_debug/control
# повторить неудачную команду, затем посмотреть: dmesg | tail
echo "module amneziawg -p" > /sys/kernel/debug/dynamic_debug/control
```

**Параметр нельзя снять через `awg setconf` или `awg syncconf`.** Если убрать строку из конфига и применить его заново, значение останется прежним: для AWG-параметров эти команды работают на добавление. Вернуть интерфейс к чистому состоянию можно только пересозданием, то есть `systemctl restart awg-quick@awg0` (или `awg-quick down` и `up`).

Начиная с v5.25.0 скрипты управления это отслеживают и **предупреждают**: они запоминают набор параметров интерфейса, применённый в прошлый раз, и если из `awg0.conf` какой-то параметр пропал, в журнале появляется предупреждение `Из секции [Interface] убрано: S1` вместе с командой, которая доводит снятие до живого интерфейса. Сам перезапуск скрипт **не делает**: он оборвал бы соединения всех клиентов, а решение такой цены принимает человек, а не автоматика. Предупреждение обычно показывается один раз: снимок набора обновляется после успешного применения, а также после `manage restart`, `manage restore` и установки, то есть после всего, что пересоздаёт интерфейс. Если же применение упало, снимок остаётся прежним и предупреждение повторится на следующем запуске - это сделано намеренно, иначе неудача молча погасила бы напоминание о том, что на живом интерфейсе параметр ещё стоит. Значения параметров по-прежнему применяются без перезапуска: `syncconf` меняет их корректно, проблема только в снятии.

<a id="awg3-not-yet-adv"></a>
### Чего установщик пока не включает и почему

Возможностей 3.0 в генерируемых конфигах нет, и это осознанно:

- **`HeaderProtectionKey`** требует, чтобы его понимали ВСЕ ваши клиенты одновременно. На момент выхода этой версии `amneziawg-android` 3.0.1 остаётся предрелизом, а у `amneziawg-windows-client` последний выпуск - 2.0.2. Включить его сейчас значит отрезать часть устройств без предупреждения.
- **`ContentPaddingAddition`** ждёт исправления в upstream: сейчас паддинг ломает распознавание служебных пакетов, они отбрасываются с сообщением `Packet is neither ipv4 nor ipv6` ([issue #186](https://github.com/amnezia-vpn/amneziawg-linux-kernel-module/issues/186)).
- **Параметры таймингов** односторонние и безопасны, но пользы без остальной картины дают мало, поэтому идут вместе со следующим шагом.

Отдельный разбор ситуации на ядрах старше 6.7 - в разделе <a href="#debian-support-adv">Поддержка Debian</a>.
...
### Что видно сайту при делении трафика по адресу назначения

Деление по адресу назначения выбирает выход для соединения по тому, куда оно направлено. Так устроены [каскад из двух серверов](CASCADE.md), [схема с WARP для российского сегмента](WARP-RU.md) и клиентский список `AllowedIPs`, из которого часть сетей исключена намеренно - например, чтобы российские сайты открывались мимо VPN (это режим 3 выше; режим 2 сюда не относится, он отправляет в туннель практически весь публичный IPv4).

Адрес посетителя сайт узнаёт не только из соединения. На странице может работать скрипт, который спрашивает адрес у стороннего сервиса определения IP - а запрос к сервису идёт уже по другому адресу, значит может уйти и другим плечом, чем сам сайт. Тогда сервис ответит адресом того выхода, которым до него дошли, скрипт передаст ответ сайту, и у сайта окажется два разных адреса: source IP соединения и то, что сообщил скрипт.

Расхождение возникает не всегда: всё решает то, по какую сторону деления попал сам сервис. Оказался на том же плече, что и сайт - адрес будет один. Загвоздка в том, что выбираете это не вы, а по цифрам ниже видно, что в проверенной выборке почти все такие сервисы лежали по «зарубежную» сторону.

Следствия у трёх схем разные:

- **клиентский список с исключениями.** Сайт видит домашний адрес, а запрос к сервису уходит через туннель, так что сайту достаются оба адреса - домашний и серверный;
- **каскад.** Сайт видит адрес входного сервера, скрипт сообщит адрес зарубежного выхода. Домашний адрес при этом не раскрывается: оба адреса ваши серверные (при условии, что IPv6 устройства не идёт мимо туннеля - см. ниже);
- **схема с WARP.** Сайт видит адрес Cloudflare, а скрипт - адрес самого сервера, то есть ровно тот адрес дата-центра, от которого схема и уводила.

Дописать сервисы определения адреса в список исключений - путь ненадёжный. Проверка 7 августа 2026 против снимка российских сетей в репозитории (`cascade/ru.zone`, 8626 сетей): взял 24 домена популярных сервисов такого рода, разрешил каждый через два резолвера (`dig @8.8.8.8` и `@77.88.8.8`) и прогнал полученные адреса по снимку.

- внутри снимка оказались два домена - `internet.yandex.ru` и `ipinfo.yandex.ru`, то есть один и тот же сервис под двумя именами, с общим адресом. Остальные 22 лежали вне российских сетей, значит запрос к ним уходит «зарубежным» плечом;
- имя в зоне `.ru` о самом адресе не говорит ничего: `2ip.ru` и `myip.ru` отвечали адресами, которых в снимке нет;
- адреса вообще непостоянны. `api.ipify.org` за одни сутки отдал три разных адреса, и разные резолверы отвечают по-разному: это anycast за Cloudflare. Прописать такое в список один раз нельзя.

Проверку легко повторить: адреса из `dig` прогнать по `cascade/ru.zone` модулем `ipaddress` из стандартной библиотеки Python. ⚠️ Мерилось против того снимка, которым пользуется каскад. У схемы с WARP список свой - BGP-фид antifilter, и его содержимое я так не проверял: механика от этого не меняется, но конкретные числа к нему не относятся.

Чего эти цифры НЕ доказывают. Насколько часто сайты действительно сравнивают адрес соединения с тем, что сообщил скрипт, я не измерял. И само расхождение доказательством VPN не является: у dual-stack посетителя без всякого VPN сайт может открыться по IPv6, а сервис определения адреса ответить по IPv4. Проверено здесь только то, что при делении трафика расхождение может возникнуть и что снаружи его видно.

Если сайт должен видеть один и тот же адрес, деление по адресу назначения этого не гарантирует, где бы оно ни стояло. Убирает деление полный туннель (`--route-all`): тогда весь трафик уходит одним выходом. Оговорка: разные адреса IPv4 и IPv6 у одного и того же сервера при этом остаться могут, но это обычная картина для любого dual-stack подключения и признаком деления трафика не является. А если адрес нужен именно российский - значит и сервер нужен в России.

**IPv6 - отдельный уровень, и его легко перепутать.** В гайдах фраза «IPv6 выключен, установщик так и ставит по умолчанию» означает IPv6 **на сервере** (sysctl хоста). IPv6 **на вашем устройстве** установщик не трогает вовсе, а деление трафика к нему не применяется: и список российских сетей у каскада, и BGP-фид у схемы с WARP содержат только IPv4 (в снимке `cascade/ru.zone` IPv6-записей нет ни одной). Пойдёт ли IPv6 устройства мимо туннеля, решает не схема, а то, попал ли в `AllowedIPs` маршрут `::/0`:

- **полный туннель без `--allow-ipv6-tunnel`** (поведение по умолчанию): клиент получает `0.0.0.0/0, ::/0`, и IPv6 устройства уходит в туннель. Сам туннель IPv6 не несёт, поэтому дальше такой трафик не идёт - но и мимо VPN не утекает;
- **раздельная маршрутизация** (режимы 2 и 3, режим 2 - по умолчанию): `::/0` не добавляется никогда, иначе сломалось бы само деление. IPv6 устройства идёт мимо туннеля со своим настоящим адресом, и тогда сайту не нужны никакие скрипты - он видит этот адрес напрямую;
- **включён `--allow-ipv6-tunnel`**: правила другие и зависят от того, есть ли у сервера нативный IPv6. Единственное их описание - [IPv6 dual-stack в туннеле](#ipv6-tunnel-adv); там же случай, когда клиенту достаётся только туннельная ULA, а глобальный IPv6 устройства снова идёт мимо.

Проверка с подключённого устройства: `curl -6 ifconfig.co`. Смотреть надо не на факт ответа, а на сам адрес - при работающем IPv6 в туннеле ответ тоже придёт. Вернулся ваш домашний адрес - значит IPv6 идёт мимо туннеля. Нужна гарантия, что мимо не идёт ничего, - выключайте IPv6 на самом устройстве: серверный фильтр здесь не поможет, прямой IPv6-трафик до сервера просто не доходит.
...
> **IPv6 в IPv4-only режиме (по умолчанию).** Когда туннель работает только по IPv4, IPv6-трафик вашего устройства идёт напрямую, мимо VPN, - так и задумано: IPv4-only туннель по определению не несёт IPv6, и сервер на это не влияет (это свойство режима, а не утечка серверной стороны). Одна оговорка: сказанное верно для раздельной маршрутизации (режимы 2 и 3). В полном туннеле (`--route-all`) клиент получает `::/0` рядом с `0.0.0.0/0`, поэтому IPv6 устройства уходит в туннель и там глохнет, а не идёт напрямую - разбор в разделе [Что видно сайту при делении трафика по адресу назначения](#split-detect-adv). Нужен IPv6 в туннеле - включите `--allow-ipv6-tunnel` (ниже). Нужна, наоборот, гарантия, что ничего не идёт мимо VPN, - отключите IPv6 на самом устройстве: серверный фильтр тут не поможет, потому что прямой IPv6-трафик до сервера просто не доходит.
...
### Смена серверного ключа

Понадобится, если приватный ключ сервера куда-то утёк: попал в скриншот, в сообщение на форуме или в чужие руки вместе с доступом. Отдельной команды для этого нет, но переустанавливать сервер не нужно.

Сначала о том, чего бояться не стоит. **Записанный ранее трафик расшифровать не смогут** - за это отвечает forward secrecy рукопожатия, а из статического ключа сервера сессионные не выводятся. Реальный риск другой: владея ключом, можно **выдать себя за ваш сервер** перед клиентом, который ему доверяет. Поэтому смена ключа закрывает проблему не в момент замены на сервере, а тогда, когда **каждый клиент получит новый конфиг**.

Ключ живёт в трёх местах, и менять надо все три сразу:

| Файл | Что в нём |
|---|---|
| `/etc/amnezia/amneziawg/awg0.conf` | строка `PrivateKey` в секции `[Interface]` |
| `/root/awg/server_private.key` | тот же приватный ключ отдельным файлом |
| `/root/awg/server_public.key` | публичный ключ, из него собираются конфиги клиентов |
...
# 3. Подставить приватный ключ в конфиг сервера.
#    В секции [Peer] строки PrivateKey нет, поэтому первая по файлу - нужная.
#    awk с -v безопасен для ключей: в base64 есть / и +, и экранировать их
#    не требуется, в отличие от sed.
...
# 4. Перезапустить сервис и перевыпустить конфиги всех клиентов
systemctl restart awg-quick@awg0
bash /root/awg/manage_amneziawg.sh regen
```

`regen` перепишет конфиги всех клиентов с новым публичным ключом, заново создаст QR-коды и ссылки `vpn://`, а приватные ключи клиентов, их адреса и `AllowedIPs` оставит как были. После этого **раздайте новые конфиги заново** - до этого момента старые не работают, и именно они остаются уязвимыми.

⚠️ **Три места, где легко ошибиться:**

- **Пропустить `server_private.key`.** Если поменять ключ только в `awg0.conf`, переустановка с `--force` вернёт старый: серверные ключи генерируются только при отсутствии этого файла, а конфиг собирается из него.
- **Пропустить `regen`.** Файлы `.conf`, `.png`, `.vpnuri` и `.vpnuri.png` у существующих клиентов останутся со старым публичным ключом. Кто импортирует по QR или по ссылке, получит нерабочий конфиг.
- **Восстановить старый бэкап позже.** Архивы в `/root/awg/backups/` содержат и `awg0.conf`, и оба файла ключей, поэтому восстановление отменит ротацию молча. Старые архивы после смены ключа лучше убрать.

Откат: восстановить три файла из бэкапа, перезапустить сервис и **обязательно снова выполнить `regen`** - конфиги клиентов к этому моменту уже переписаны новым ключом.
...
Опции:
  ...
  --port=НОМЕР          Установить UDP порт (1-65535; на мобильных сетях с DPI часто выручает 443/udp)
...
Опции:
  ...
  --json                Машиночитаемый JSON-вывод (v5.21.0: большинство команд, см. раздел «JSON-интерфейс»)
  ...
  --reset-routes        (только для regen) сбросить AllowedIPs клиентов на глобальный режим маршрутизации
```
...
| `AWG_STRICT_CONFIRM=1` | Неинтерактивный запуск деструктивной команды без `--yes`/`AWG_YES=1` отказывает (rc 1) вместо тихого согласия. Для cron/CI/ботов (v5.21.0) |
...
* **`regen [имя ...] [--reset-routes]`:** Перегенерировать файлы `.conf`/`.png` для указанных клиентов или всех сразу. По умолчанию сохраняет индивидуальные `AllowedIPs`/`DNS`/`PersistentKeepalive` клиента (заданные через `modify`). С `--reset-routes` - сбрасывает `AllowedIPs` на текущий глобальный режим маршрутизации из `awgsetup_cfg.init`; используйте после смены режима переустановкой (`--force --route-all` / `--route-amnezia` / `--route-custom=`), чтобы новый режим дошёл до существующих клиентов (Issue #170).
...
<a id="json-api-adv"></a>
## 🤖 JSON-интерфейс для автоматизации (--json)

С v5.21.0 флаг `--json` поддерживают не только `list`/`stats`, но и команды управления: `add`, `remove`, `regen`, `modify`, `backup`, `restore`, `check` (алиас `status`), `restart`, `repair-module` (алиас `repair`). Правила контракта:

- **stdout = ровно один JSON-документ** одной строкой, при любом исходе, включая ошибки. Человеческие сообщения уходят в stderr. Читайте stdout целиком - потокового вывода нет, но у batch-команд `results[]` растёт линейно с числом имён.
- **Exit-код - источник истины.** Поле `ok` дублирует его для ботов, читающих только stdout: `ok=false` при любом провале, в том числе частичном (`add a b`, где `b` уже существует).
- **Совместимость аддитивная:** новые поля могут добавляться; существующие не переименовываются и не меняют тип. Наборы `status`-значений могут расширяться - неизвестное значение трактуйте как ошибку соответствующей записи. `list`/`stats` заморожены как есть (голые массивы, без конверта).
- **Поле `error` - человекочитаемый текст** (может быть локализован), не парсите его; машинные решения принимайте по `ok`, `rc` и `status`.
- **Алиасы канонизируются:** в ответе всегда `"command":"check"` и `"command":"repair-module"`, как бы вы ни набрали команду.
- `--json` **не подразумевает** `--yes`: деструктивные команды по-прежнему требуют подтверждения.
- `help`, `show`, `diagnose` флаг не поддерживают - их вывод остаётся человеческим.
...
Статусы записей `results[]`: `add` - `created|exists|invalid_name|error`; `remove` - `removed|not_found|invalid_name|error`; `regen` - `regenerated|not_found|invalid_name|error`.
...
По умолчанию неинтерактивный запуск (cron, CI, бот) деструктивной команды молча продолжается, как будто вы ответили «да» - это историческое поведение, и оно сохранено. `AWG_STRICT_CONFIRM=1` включает строгий режим: без явного `--yes` (или `AWG_YES=1`) команда откажется с rc 1, а с `--json` вернёт `{"ok":false,"error":"AWG_STRICT_CONFIRM=1: non-interactive run requires --yes","rc":1}`.

```bash
# Рецепт для ботов: строгий режим + явное согласие
AWG_STRICT_CONFIRM=1 bash manage_amneziawg.sh remove phone --json --yes
```

Действует на один запуск, в конфиг не сохраняется. Активирует строго значение `1`.
...
<details>
  <summary><strong>В: Российские сайты не открываются через VPN, потому что не пускают адреса зарубежных дата-центров. Что делать, если сервер один?</strong></summary>
  <b>О:</b> Завернуть только российский сегмент во второй выход через Cloudflare WARP, а остальной трафик оставить прямым. Список российских сетей сервер получает автоматически по BGP, вести его руками не нужно. Российские сайты увидят адрес Cloudflare вместо адреса дата-центра. Пошаговая инструкция - <a href="WARP-RU.md">WARP-RU.md</a>. Если нужен именно российский IP, а не адрес Cloudflare, потребуется второй сервер в РФ и каскад (<a href="CASCADE.md">CASCADE.md</a>).
</details>
...
<details>
  ...
  <b>Важно:</b> совпадать на сервере и всех клиентах обязаны S1-S4 и H1-H4, иначе handshake не пройдёт; Jc/Jmin/Jmax и I1-I5 совпадать не обязаны. Для генерации новых случайных непересекающихся H1-H4 диапазонов проще всего переустановить сервер (<code>--uninstall</code> + повторная установка) - каждая установка генерирует уникальный набор.
</details>
...
Раз ванильный WireGuard работает, сеть, порт и фаервол можно вычеркнуть. Разница между ним и AmneziaWG 2.0 ровно одна - слой обфускации: `Jc`/`Jmin`/`Jmax`, `S1`-`S4`, `H1`-`H4` и, в версии 2.0, `I1`-`I5`. Разбор входящего пакета опирается только на `S1`-`S4` и `H1`-`H4`: если хоть один из них на сервере и клиенте не совпадает байт-в-байт, сервер не может разобрать handshake и молча отбрасывает его - снаружи это выглядит как «пакеты идут, но не обрабатываются».

Что проверить:

1. Сверьте параметры обфускации в секции `[Interface]` на сервере и у клиента. Совпадать обязаны `S1`-`S4` и `H1`-`H4`: приёмник срезает этот паддинг и проверяет эти заголовки, поэтому любое расхождение здесь означает, что пакет отброшен и рукопожатие не сойдётся. В 2.0 `H1`-`H4` задаются диапазонами - диапазоны должны быть одинаковыми с обеих сторон. А `Jc`/`Jmin`/`Jmax` и `I1`-`I5` совпадать не обязаны: это отдельные пакеты-обманки, вторая сторона их отбрасывает, а узел без `I1` просто не шлёт их вовсе, и рукопожатию это не мешает. Регистр у `I1`-`I5` при этом важен: только верхний.
...
В конце июля 2026 команда Amnezia выпустила **AmneziaWG 3.0** и переключила на него PPA `ppa:amnezia/ppa`. Поначалу модуль 3.0 не собирался на ядрах старше 6.7: он вызывал функцию `nla_put_uint`, которой в Linux до версии 6.7 нет, и на **Debian 12 (bookworm) с ядром 6.1** установка обрывалась на `implicit declaration of function 'nla_put_uint'`. 31 июля разработчики это исправили ([issue #204](https://github.com/amnezia-vpn/amneziawg-linux-kernel-module/issues/204)): в версии модуля `v3.0.20260731-04` сборка на ядре 6.1 проходит, проверено на Debian 12 с ядром 6.1.0-51.

Инсталлятор при этом продолжает держать на таких ядрах **закреплённый последний модуль AmneziaWG 2.0** (тег `v1.0.20260725`, сверяется по commit-хэшу): начиная с **v5.23.0**, если версия ядра старше 6.7, модуль не берётся из PPA, а собирается из исходников через DKMS. Теперь это сознательный выбор, а не обход поломки: линия 3.0 в первые же дни успела сломать и починить сборку именно на старых ядрах, то есть там она обкатана меньше всего. Утилиты `amneziawg-tools` по-прежнему ставятся из PPA - версия 3.0 корректно работает с модулем 2.0, это проверено. Ставить что-либо вручную не нужно, всё происходит на шаге 2. На ядрах 6.7 и новее (Ubuntu 24.04/25.10/26.04, Debian 13 trixie) поведение прежнее - модуль ставится из PPA.

Если хотите получить на Debian 12 именно AmneziaWG 3.0, проще всего развернуть сервер заново на Debian 13 / Ubuntu 24.04+. Второй путь - поставить ядро 6.7+ из `bookworm-backports` и **перезагрузиться в него**: инсталлятор смотрит на работающее ядро, а не на установленное, и после перезагрузки пойдёт обычным путём. На уже настроенном сервере не ставьте поверх: сделайте резервную копию (`sudo bash /root/awg/manage_amneziawg.sh backup`), снимите установку (`sudo bash install_amneziawg.sh --uninstall`) и поставьте заново - иначе закреплённый модуль 2.0 останется зарегистрированным в DKMS под тем же именем, что и пакет из PPA. Поддержка самих возможностей 3.0 (header protection, рандомизация таймингов) в инсталляторе планируется отдельно и появится, когда стек 3.0 и клиентские приложения стабилизируются.
...
Создайте `/etc/amnezia/amneziawg/awg0.conf` - замените `YOUR_*` на свои значения. Ключи генерируются через `awg genkey | tee /etc/amnezia/amneziawg/server_private.key | awg pubkey`. Из параметров обфускации совпадать с клиентом обязаны `S1`-`S4` и `H1`-`H4`; `Jc`/`Jmin`/`Jmax` и `I1` совпадать не обязаны:
